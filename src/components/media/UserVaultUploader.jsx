...
const UserVaultUploader = ({totalSize,getAllMedia, apiData}) => {
  ...
  const uploadFiles = () => {
    ...
    var countRes = 0
     for (let i = 0; i <filesToUpload.length; i++) {
      ...
      if(folderName){
        data.append('folder_name', `${folderName}-userVault`);
      }else{
        data.append('folder_name', `${apiData?.user?.username}-userVault`);
      }
...
export default UserVaultUploader