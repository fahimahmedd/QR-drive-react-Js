...
const UserVaultMyItem = ({ data, onClick,getAllMedia, allVaultLists }) => {
  ...
  const updateFolderName = folderName.split('-userVault').join(" ")
    ...
    return (
      <Fragment>
        <div className={segName ==='user-vault'?'my_item_user_vault':'my_item'}>
          ...
          {updateFolderName && (
            <div className="item_name"> {updateFolderName.split("-").join(" ")}</div>
          )}
...
export default UserVaultMyItem;
